...
const EDIT_PASSWORD = "pop123";

let data = null;
let activeCategory = "apps";
let adminMode = false;
let editorUnlocked = sessionStorage.getItem("sadecki_editor_unlocked") === "yes";
...
function openEditor() {
  if (!editorUnlocked) {
    const entered = prompt("Enter editor password:");
    if (entered !== EDIT_PASSWORD) {
      if (entered !== null) alert("Wrong password.");
      return;
    }

    editorUnlocked = true;
    sessionStorage.setItem("sadecki_editor_unlocked", "yes");
  }
